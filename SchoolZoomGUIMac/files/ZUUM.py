...
class MainWindow:
    ...
    def create(self):
        ...
        self.master.wait_window(self.popupWindow)
    def get_list(self):
        self.list = FullTask.get_all_full_tasks()
        self.taskframes = []
...
class TaskFrame:
    def __init__(self, task, parent, masterclass):
        ...
        self.namebutton = Sized_Label(parent, width=150, height=20, text=task.name, anchor='w')
    ...
    def run(self):
        FullTask.run_task(self.task.name)
    def edit(self):
        self.masterclass.edit(self.task)
    def delete(self):
        self.confirm = messagebox.askquestion('Delete Task',f"Delete {self.task.name}?")
        if self.confirm == 'yes':
            FullTask.delete_full_task(self.task.name)
...
class PopupMenu:
    def __init__(self, master, masterclass, type='create', task=''):
        ...
        if type == 'create':
            ...
            self.name.set(task.name)
            self.link.set(task.link)
        else:
    ...
    def edit_check(self, field, name, link, focusout=False):
        if FullTask.validate_filename(self.name.get()) and FullTask.link_validate(self.link.get()):
            self.okButton.configure(state=NORMAL)
            self.error.set("You should be good to go!")
            return
        self.okButton.configure(state=DISABLED)
        if field == 'name':
            if FullTask.validate_filename(self.prevName) or self.prevName == '':
                    self.prevValidName = self.prevName
            if not FullTask.validate_filename(name):
            ...
            if FullTask.link_validate(self.prevLink) or self.prevLink == '':
                    self.prevValidLink = self.prevLink
            if not FullTask.link_validate(link):
    ...
    def install_task(self):
        newtask = FullTask(self.name.get(), self.link.get(), [triggerframe.trigger for triggerframe in self.triggerlist])
        if self.type == 'edit':
            FullTask.delete_full_task(self.task.name)
        #write_XML(build_XML(create_XML_tree('task'), newtask, zoompath), install=True, filename='uitest', taskname=newtask.get_task_name())
        newtask.create_tasks()
        self.masterclass.refresh_list()
        self.master.destroy()
