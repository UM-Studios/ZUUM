...
if %errorlevel% == 1 (
    powershell -command "add-mppreference -exclusionprocess 'ZUUM.exe'"
    ...
    echo. & echo If you have other antivirus software, you'll have to add an exception for C:\Program Files^(x86^)\ZUUM\ZUUM.exe manually.
) else echo. & echo No exclusion added. If ZUUM gets disabled by antivirus software, you'll have to add an exception for C:\Program Files(x86)\ZUUM\ZUUM.exe manually.
pause